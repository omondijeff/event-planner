<template>
  <div class="flex flex-col w-64 h-screen text-white bg-gray-900">
    <!-- Logo Section -->
    ...
    <nav class="flex-1 px-4 py-6 space-y-2">
      ...
      <!-- Venue Explorer Button (accessible for all roles) -->
      <div>
        <RouterLink
          to="/venue-explorer"
          class="flex items-center px-3 py-2 text-sm font-medium rounded-md hover:bg-gray-700"
          :class="{ 'bg-gray-800': $route.path === '/venue-explorer' }"
        >
          <BuildingStorefrontIcon class="w-5 h-5 text-gray-400" />
          <span class="ml-3">Venue Explorer</span>
        </RouterLink>
      </div>
    </nav>
    ...
    <div class="px-4 py-6 border-t border-gray-700">
      <button
        @click="logout"
        class="flex items-center justify-center w-full px-4 py-2 text-sm font-medium text-white bg-red-600 border border-transparent rounded-md shadow-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500"
      >
